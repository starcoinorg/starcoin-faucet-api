...
def scrape_twitter(id, url, created_at, db: Session, faucet):
    ...
    if not address:
        faucet_crud.faucet.update(db, db_obj=faucet, obj_in={
                                  "status": FaucetStatus.fail.value})
        return False, id, "none address found"

    count = faucet_crud.faucet.get_day_count_by_address(
        db=db, network=faucet.network, address=address, since=created_at)
    ...
    try:
        ...
        logger.info({"address": address, "status": FaucetStatus.success.value,
