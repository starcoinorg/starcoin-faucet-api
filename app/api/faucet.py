...
async def create(url: str, network: str = FaucetNetwork.default, db: Session = Depends(deps.get_db)):
    ...
    if validate_url(url) is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="None validate URL")

    url = normalise_query_string(url)
    platform = get_platform(url)

    if not platform:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Wrong platform URL")

    #username = ''
    #if platform == FaucetPlatform.twitter:
    #    username = get_twitter_username(url)

    count = faucet_crud.faucet.get_day_count_by_user(db=db, url=url, network=network, platform=platform)
    if count > 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="dup url, try tomorrow")


    # !! dup record fliter
    obj_in = Faucet(url=url, network=FaucetNetwork[network], platform=platform, amount=FaucetAmount[FaucetNetwork.default].value["num"])
    #print(obj_in)
